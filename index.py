...
class Bill_App:
    def __init__(self,root):
        ...
        #=======
        cock_lbl = Label(F2,font = ("times new roman",15,"bold"),fg = lbl_color,bg = bg_color,text = "Coldrink")
...
#Function to get total prices
    def total(self):
        ...
        self.tax_cos.set("Rs. "+str(round(self.total_food_prices*0.18)))
        #====================Total Grocery Prices
        self.total_grocery_prices = (
            (self.wheat.get()*100)+
            (self.food_oil.get() * 200)+
            (self.salt.get() * 55)+
            (self.rice.get() *50)+
            (self.sugar.get() * 80)
        ...
        self.tax_groc.set("Rs. "+str(round(self.total_grocery_prices*0.12)))
        #======================Total Other Prices
        self.total_other_prices = (
            (self.gatorade.get() * 80)+
            (self.juice.get() * 80)+
            (self.coke.get() * 80)+
            (self.waffer.get() * 30)+
        ...
        self.tax_other.set("Rs. "+str(round(self.total_other_prices*0.20)))
...
#Add Product name , qty and price to bill area
    def bill_area(self):
        self.welcome_soft()
        if self.bread.get() != 0:
            self.txt.insert(END,f"\nBread             {self.bread.get()}           {self.bread.get() * 35}")
        if self.candy.get() != 0:
            self.txt.insert(END,f"\nCandy             {self.candy.get()}           {self.candy.get() * 30}")
        if self.hamburger.get() != 0:
            self.txt.insert(END,f"\nHamburger         {self.hamburger.get()}           {self.hamburger.get() * 80}")
        if self.hotdog.get() != 0:
            self.txt.insert(END,f"\nHotdog            {self.hotdog.get()}           {self.hotdog.get() * 60}")
        if self.sandwich.get() != 0 :
            self.txt.insert(END,f"\nSandwich          {self.sandwich.get()}           {self.sandwich.get() * 40}")
        if self.wheat.get() != 0:
            self.txt.insert(END,f"\nWheat             {self.wheat.get()}           {self.wheat.get() * 100}")
        if self.food_oil.get() != 0:
            self.txt.insert(END,f"\nFood Oil          {self.food_oil.get()}           {self.food_oil.get() * 200}")
        if self.salt.get() != 0:
            self.txt.insert(END,f"\nSalt              {self.salt.get()}           {self.salt.get() * 55}")
        if self.rice.get() != 0:
            self.txt.insert(END,f"\nRice              {self.rice.get()}           {self.rice.get() * 50}")
        if self.sugar.get() != 0:
            self.txt.insert(END,f"\nSugar             {self.sugar.get()}           {self.sugar.get() * 80}")
        if self.gatorade.get() != 0:
            self.txt.insert(END,f"\nGatorade          {self.gatorade.get()}           {self.gatorade.get() * 80}")
        if self.juice.get() != 0:
            self.txt.insert(END,f"\nJuice             {self.juice.get()}           {self.juice.get() * 80}")
        if self.coke.get() != 0:
            self.txt.insert(END,f"\nCoke              {self.coke.get()}           {self.coke.get() * 80}")
        if self.waffer.get() != 0:
            self.txt.insert(END,f"\nWaffer            {self.waffer.get()}           {self.waffer.get() * 30}")
